function  UpdateSPOHubSiteAssociationRequest {
  [CmdletBinding()]
  param
  (
  ...
  process {
    if ($Site) {
      ...
      if ($siteUpdateRequestType -eq "Hub Site Association") {  

        #Check if Hub site is already exists
        $chkHUBSite = Get-PnPHubSite -Identity $existingHUBsiteURL
        if ($chkHUBSite.SiteId) {

          #Proceed with Hub Site Association if HubSite is Present otherwise it will skip
          Add-PnPHubSiteAssociation -Site $siteUrl -HubSite $existingHUBsiteURL @Params 
          Log-Message "Associated HubSite with ExistingHubSiteURL successfully!!" $logfile Success
          $addScriptResponse.Add("Associated HubSite with ExistingHubSiteURL SiteCollection URL : $siteUrl ") 
          $RegisterHubsite = $true

        }

        else {
          Log-Message "Unable to Associate HubSite due to the existing HUBSite Url is Invalid or may be its not HUBsite" $logfile Error
          $addScriptResponse.Add("Unable to Associate HubSite due to the existing HUBSite Url is Invalid or may be its not HUBsite")
      ...
      Log-Message "Unable to Associate HubSite due to the requested site does not exist" $logfile Error
      $createErrorList = $true
    ...
    if (!$createErrorList) {
      ...
      Set-PnPListItem -List SiteUpdateTracker -Identity $listItemIdentity -Values @{ 
        ...
        "ScriptResponse"        = "SPO HubSite association Change request has been updated Successfully"; 
      ...
      if ($scriptAttempt -le "4") {
        ...
        Set-PnPListItem -List SiteUpdateTracker -Identity $listItemIdentity -Values @{
          ...
          "ScriptAttempts"     = $scriptAttempt;
          "ScriptResponse"     = "SPO HubSite association Change request has been failed to Update";
          "ScriptHistory"      = $addScriptResponse  
        }
      }
      ...
      if ( ($scriptAttempt -eq "5") -or ($addScriptResponse -eq 'Requested Site does not exists')) {
        Set-PnPListItem -List SiteUpdateTracker -Identity $listItemIdentity -Values @{ 
          ...
          "ScriptAttempts"     = $scriptAttempt;
          "ScriptResponse"     = "SPO HubSite association Change request has been Rejected";
          "ScriptHistory"      = $addScriptResponse 
        }
  ...
  end { Log-Message "SPO HubSite association Change request has been completed Successfully!!" $logfile Success }
}
